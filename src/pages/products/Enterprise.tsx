...
const EnterpriseService = () => {
  ...
  const [productList] = createResource(
    () => t('selectedLanguage'),
    (langKey) => productService.getEnterprisePlans(langKey),
  );
  ...
  const formatPrice = (price: { monthly: number } | string) => {
    ...
    return t('selectedLanguage') == 'id-ID'
      ? `Rp.${formattedPrice.toLocaleString('id-ID')}k`
      : `$${formattedPrice}`;
  };
      ...
      <section class="bg-white py-12 px-4">
        <div class="max-w-7xl h-[48rem] mx-auto flex flex-col lg:flex-row items-center gap-12">
          ...
          <div class="text-center lg:text-left max-w-xl">
            ...
            <p class="text-gray-600 mb-6 text-lg">{t('enterprise.description')}</p>
          </div>
        </div>
      </section>
          ...
          <div class="grid md:grid-cols-2 gap-8">
            ...
            <div class="bg-white p-8 rounded-lg shadow-sm">
              <h3 class="text-2xl font-semibold mb-4">
                {t('enterprise.services.integration.title')}
              </h3>
      ...
      <section class="py-12 px-4">
        <div class="max-w-7xl mx-auto">
          ...
          <h3 class="text-xl md:text-xl font-bold text-center mb-8">
            <span class="text-gray-500 line-through">{basePrice()}</span>
            <br />
                ...
                {(plan) => (
                  <div class="bg-white p-8 rounded-lg shadow-md text-center flex flex-col h-full">
                    ...
                    <div class="h-24">
                      <p class="mb-4">
                        ...
                        <span class="text-lg font-normal">
                          {plan.name !== 'Plus' ? `/${t('enterprise.monthlyShort')}` : ''}
                        </span>
                      </p>
                      <Show when={plan.price.original_monthly}>
                        <p class="text-gray-500 line-through">
                          {isYearly()
                            ? formatPrice({ monthly: plan.price.original_monthly * 9 })
                            : formatPrice({ monthly: plan.price.original_monthly })}
                          <span class="text-lg font-normal">
                            {plan.name !== 'Plus' ? `/${t('enterprise.monthlyShort')}` : ''}
                          </span>
                        </p>
                    ...
                    <div class="flex flex-col mt-auto">
                      <a
                        href="/about"
                        class="bg-[var(--color-primary)] text-[var(--color-customDark)] px-6 py-3 rounded hover:bg-gray-700 transition"
                      >
                      ...
                      <Show when={plan.demoUrl}>
                        <a
                          ...
                          class="mt-4 inline-flex items-center justify-center px-6 py-3 text-base font-medium text-center text-gray-900 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 focus:ring-4 focus:ring-gray-100"
                        >
                          <svg
                            class="w-5 h-5 mr-2"
                            fill="currentColor"
                          ...
                          >
                            ...
                            <path
                              fill-rule="evenodd"
                              d="M.458 10C3.732 4.943 7.522 3 10 3s6.268 1.943 9.542 7c-3.274 5.057-7.03 7-9.542 7S3.732 15.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z"
                              clip-rule="evenodd"
                            ></path>
                          </svg>
                          {t('shopify.viewDemo')}
