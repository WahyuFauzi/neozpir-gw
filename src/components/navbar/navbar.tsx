  ...
  return (
    <nav class="bg-white border-gray-200 sticky top-0 z-50">
      <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <div class="flex items-center space-x-3 rtl:space-x-reverse w-1/2 lg:w-auto">
          <a href="/" class="flex items-center space-x-3 rtl:space-x-reverse">
            <span class="self-center text-2xl font-semibold whitespace-nowrap text-[#3DDC97]">Neozpir</span>
          </a>
        </div>
        <div class="flex lg:hidden items-center justify-end w-1/2">
        ...
        <div class={`w-full mx-auto lg:block lg:w-auto ${menuOpen() ? 'block' : 'hidden'}`} id="navbar-default">
          <ul class="font-medium flex flex-col p-4 lg:p-0 mt-4 lg:flex-row lg:space-x-8 rtl:space-x-reverse lg:mt-0 lg:border-0">
            <li>
            ...
            <Show when={!auth()?.session}>
              <li class="max-w-32 rounded-full outline-2 outline-gray-500/50 cursor-pointer text-center my-2 md:my-0 lg:hidden">
                <a href="/login" class="block py-2 px-3 hover:bg-gray-200 rounded-full" onClick={() => setMenuOpen(false)}>{t('navbar.login')}</a>
              </li>
              <li class="max-w-32 rounded-full cursor-pointer text-center my-2 md:my-0 lg:hidden">
